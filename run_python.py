...
for train_data in train_dataset_groups:
    ...
    dataset_config_path = "datasets/2024-06-04_15-35-37/dataset_config.json"
    model_key = 'resnet18'
    transform_type = 'standard'
    arguments = ['--model_key', model_key, '--transform_type', transform_type, '--train_dataset_paths', *train_paths, '--validation_dataset_paths', *val_paths, '--wandb_project_name', 'test_project',
                '--dataset_config_path', dataset_config_path, '--lr', '0.00009354747253832916', '--batch_size', '128', '--pretrained', 'True', '--augmentation', 'True', '--shuffle', 'True', '--epochs', '30', '--wandb_run_id', str(uuid4())]
    ...
    for test_group in test_dataset_groups:
        ...
        joined_test_names = '_'.join(test_group)
