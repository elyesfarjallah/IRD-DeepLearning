...
argparser.add_argument('--model_key', type=str, help='Model key')
argparser.add_argument('--transform_type', type=str, help='Transform type')
argparser.add_argument('--train_dataset_paths', type=str, help='Path to train dataset', nargs='+')
argparser.add_argument('--validation_dataset_paths', type=str, help='Path to validation dataset', nargs='+')
argparser.add_argument('--test_dataset_paths', type=str, help='Path to test dataset', nargs='+')
argparser.add_argument('--wandb_project_name', type=str, help='Wandb project name')
argparser.add_argument('--wandb_run_name', type=str, help='Wandb run name')
...
unique_train_labels = np.unique(np.concatenate([package.get_labels() for package in train_packages], axis=0))
...
model = models_torch.get_model(model_name=args.model_key, n_classes=len(label_names), pretrained=args.pretrained)
...
run_config = {'model_key' : args.model_key, 'transform_type' : transform_type, 'train_dataset_paths' : train_dataset_paths,
              'validation_dataset_paths' : validation_dataset_paths, 'test_dataset_paths' : test_dataset_paths,
              'train_dataset_size' : len(train_dataset),
                'validation_dataset_size' : len(validation_dataset), 'test_dataset_size' : len(test_dataset),
...
model_save_folder = f'models/{args.model_key}/{args.run_name}_{args.run_id}'
...
model_logger_criterrion = metric_mapping.get_metric_by_name('bce_with_logits_loss')
averaging_metrics = [averaging_metrics, [model_logger_criterrion]]
#create observers
trainer, model_logger, averaging_evaluators, classwise_evaluators, wandb_observer = create_subjects_and_observers(is_logging_to_wandb = True, project_name = args.wandb_project_name, run_name = args.wandb_run_name,
                               run_id = args.wandb_run_id, config = run_config, tags=[], watch_gradients = False, gradients_log_freq = 0,
                               model = model, averaging_metrics = averaging_metrics, model_logger_evaluation_function = is_min, model_logger_criterrion = model_logger_criterrion,
...
test_data = trainer.test(model=model, validation_loader=test_loader, device=device)
