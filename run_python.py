...
import wandb
from data_pipeline.data_loading_utils import data_packages_to_datasets, filter_data_packages_by_labels, zero_out_packages_labels
...
train_dataset_groups = [['UKB.json', 'SES.json', '1000images.json', 'ODIR-5K.json', 'RFMiD.json', 'RFMiD2.json', 'RIPS.json']]

for train_data_group in train_dataset_groups:
    train_paths = [f'{dataset_path}/train/{path}' for path in train_data_group]
    val_paths = [f'{dataset_path}/val/{path}' for path in train_data_group]
    dataset_config_path = f"{dataset_path}/dataset_config.json"
    model_key = 'resnet18'
    transform_type = 'standard'
    wandb_run_tags = '_'.join(train_data_group).replace('.json', '')
    arguments = ['--model_key', model_key, '--transform_type', transform_type, '--train_dataset_paths', *train_paths, '--validation_dataset_paths', *val_paths, '--wandb_project_name', 'dataset_expansion',
                '--dataset_config_path', dataset_config_path, '--lr', '0.00009354747253832916', '--batch_size', '128',
                '--pretrained', 'True', '--augmentation', 'True', '--shuffle', 'True', '--epochs', '50', '--wandb_run_id', f'{wandb_run_tags}_dataset_expansion', '--wandb_run_tags', wandb_run_tags]

    trainer, model, unique_train_labels = main(arguments)
    test_data = os.listdir(f'{dataset_path}/test')
    test_dataset_groups = [[test_set] for test_set in test_data]
    for test_group in test_dataset_groups:
        ...
        test_packages = zero_out_packages_labels(data_packages=test_packages, labels_to_keep=unique_train_labels)
        ...
        joined_test_names = test_group[0].replace('.json', '')
