...
def find_overrepresented_samples(file_paths: np.array, encoded_labels: np.array, max_samples_per_class : int):
    ...
    for label, samples in zip(unique_labels, samples_to_remove):
        if samples > 0:
            ...
            label_indices = np.where(np.all(encoded_labels == label, axis=1))
            #randomly select the samples to remove
            label_indices = np.random.permutation(label_indices)
            indices_to_remove = indices_to_remove.extend(label_indices[:samples])
    return indices_to_remove, file_paths[indices_to_remove], encoded_labels[indices_to_remove]
