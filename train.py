...
project_name = f'{model_key}_multilabel_classification'#wandb_config['project_name']
#initialize wand
...
def  objective(trial : optuna.Trial):
    ...
    trainer.train(model=model, criterion=criterion, optimizer=optimizer, train_loader=train_loader, validation_loader=validation_loader, n_epochs_validation=1, epochs=20, prefered_device='cuda', early_stopping=True, patience=5, min_delta_percentage=0.1)
    #load the best model
    model = torch.load(model_save_path)
    trainer.test(model=model,test_loader=test_dataloader, prefered_device='cuda')
...
study = optuna.create_study(direction='minimize', study_name=study_id)'''
#load the study
...
study.optimize(objective, n_trials=1, show_progress_bar=True, callbacks=[save_callback])
